...
class AudioSessionExportActionsTest {
    ...
    fun `export success updates status and emits snackbar`() {
        ...
        val actions = AudioSessionExportActions(
            uiState = state,
            sessionStateStore = AudioSessionStateStore(state),
            savedAudioRepository = FakeExportRepository(
                exportResult = AudioExportResult.Success(
    ...
    private fun sessionWithGeneratedAudio() = ModeAudioSessionState(
        inputText = "text",
