...
- Android package-private `C++17` native exception 继续作为 Android 独立平台偏差跟踪，但不再与已删除的 `bag/legacy/**` surface 绑定。
...
| 条目 | 最终状态 | 说明 |
| --- | --- | --- |
| Android 已从 `bag/legacy/**` 删除 blocker 中拆出 | 满足 | Android 当前仍保留 package-private `C++17` native exception，但它已不再直接或间接消费 `bag/legacy/**`；该平台偏差继续由 Android 独立 lane 跟踪，不再阻塞 legacy 删除后的维持。 |
...
- 当前已不再需要把以下问题继续登记为 blocker：
  - Android package-private `C++17` native exception 是否继续存在
