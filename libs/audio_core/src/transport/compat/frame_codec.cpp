...
std::vector<uint8_t> SliceBodyForCrc(uint8_t mode_value,
...
ErrorCode EncodeFrame(bag::TransportMode mode,
  ...
  if (!bag::IsFramedTransportMode(mode) ||
...
ErrorCode DecodeFrame(const std::vector<uint8_t>& frame_bytes,
  ...
  const size_t payload_length = (static_cast<size_t>(frame_bytes[4]) << 8) |
                                static_cast<size_t>(frame_bytes[5]);
  ...
  const std::vector<uint8_t> body(frame_bytes.begin() + 2,
                                  frame_bytes.begin() + 6 + payload_length);
  ...
  out_frame->mode = mode;
  out_frame->payload.assign(frame_bytes.begin() + 6,
                            frame_bytes.begin() + 6 + payload_length);
  return ErrorCode::kOk;
}
