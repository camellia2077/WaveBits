...
public:
    explicit UltraCompatDecoder(CoreConfig config) : config_(config) {}
    ...
    ErrorCode PollTextResult(TextResult* out_result) override {
        ...
        const ErrorCode frame_bytes_code = DecodePcm16ToFrameBytes(config_, buffered_pcm_, &frame_bytes);
        ...
        if (bag::transport::compat::DecodeFrame(frame_bytes, &frame) != ErrorCode::kOk ||
            frame.mode != TransportMode::kUltra) {
            out_result->text.clear();
            out_result->complete = false;
            out_result->confidence = 0.0f;
            out_result->mode = TransportMode::kUltra;
...
ErrorCode EncodeFrameBytesToPcm16(const CoreConfig& config,
    ...
    try {
        *out_pcm = bag::flash::EncodeBytesToPcm16(frame_bytes, bag::flash::MakeBfskConfig(config));
        return ErrorCode::kOk;
    } catch (...) {
        return ErrorCode::kInvalidArgument;
    }
...
ErrorCode DecodePcm16ToFrameBytes(const CoreConfig& config,
    ...
    try {
        *out_frame_bytes = bag::flash::DecodePcm16ToBytes(pcm, bag::flash::MakeBfskConfig(config));
        return ErrorCode::kOk;
    } catch (...) {
        return ErrorCode::kInvalidArgument;
    }
...
ErrorCode EncodeTextToPcm16Compat(const CoreConfig& config,
    ...
    std::vector<uint8_t> frame_bytes;
    const ErrorCode frame_code =
        bag::transport::compat::EncodeFrame(TransportMode::kUltra, payload, &frame_bytes);
...
std::unique_ptr<ITransportDecoder> CreateCompatDecoder(const CoreConfig& config) {
    return std::make_unique<UltraCompatDecoder>(config);
}
