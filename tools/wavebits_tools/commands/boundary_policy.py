...
_CONSUMER_SOURCE_RULES: dict[Path, _BoundaryRule] = {
    ROOT_DIR / "apps" / "audio_cli" / "windows" / "src" / "main.cpp": _BoundaryRule(
        required=(
            '#include "bag_api.h"',
            '#include "wav_io.h"',
        ),
    ...
    ROOT_DIR / "apps" / "audio_cli" / "windows" / "cmake" / "CMakeLists.txt": _BoundaryRule(
        required=(
            "bag_api",
            "audio_io",
        ),
...
_TEST_SOURCE_RULES: dict[Path, _BoundaryRule] = {
    ROOT_DIR / "Test" / "api" / "api_tests.cpp": _BoundaryRule(
        required=(
            '#include "bag_api.h"',
        ),
        forbidden=(
...
def run_phase10_test_boundary_policy_checks() -> None:
    failures: list[str] = []
...
def run_boundary_policy_checks() -> None:
    ...
    run_phase10_test_boundary_policy_checks()
